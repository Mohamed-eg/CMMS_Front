...
export const assetsAPI = {
  ...
  getAssetById: async (id) => {
    try {
      ...
      const url = buildApiUrl(API_CONFIG.ENDPOINTS.ASSETS.BY_ID, { id })
      const data = await apiRequest(url, {
        method: "GET",
      })

      console.log("✅ Asset fetched from API:", data.id)
      return data
    } catch (error) {
      console.error("❌ Error fetching asset from API:", error.message)
