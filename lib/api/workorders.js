...
export const updateWorkOrder = async (id, workOrderData) => {
  try {
    ...
    console.error("Error updating work order:", error, id)
    throw error
  }
}

// Delete a work order
export const deleteWorkOrder = async (_id) => {
  try {
    const response = await fetch(buildApiUrl(`${API_CONFIG.ENDPOINTS.WORK_ORDERS.DELETE}/${_id}`), {
      method: "DELETE",
      headers: getAuthHeaders(),
    })

    return await handleApiResponse(response).then((res) => {
      console.log(res)
      return res
    })
  } catch (error) {
...
export const getWorkOrderById = async (_id) => {
  try {
    const response = await fetch(buildApiUrl(API_CONFIG.ENDPOINTS.WORK_ORDERS.LIST + `/${_id}`), {
      method: "GET",
      headers: getAuthHeaders(),
    })
